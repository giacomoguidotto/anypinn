...
│   ├── nn.py              ← Field (MLP), Parameter, Argument, Domain
...
│   ├── samplers.py        ← CollocationSampler protocol, built-in samplers, build_sampler()
│   ├── types.py           ← Type aliases, protocols (LogFn, CollocationStrategies, etc.)
...
- **`PINNDataModule`** (`dataset.py`): Abstract Lightning DataModule. Subclasses implement `gen_data()`. Collocation points are generated by a `CollocationSampler` resolved from `TrainingDataConfig.collocation_sampler` (string literal). Handles context creation and validation resolution.
- **`CollocationSampler`** (`samplers.py`): Protocol for collocation point generation. Built-in implementations: `UniformSampler`, `RandomSampler`, `LatinHypercubeSampler`, `LogUniform1DSampler`, `AdaptiveSampler`.
- **`ResidualScorer`** (`samplers.py`): Protocol for scoring candidate collocation points by PDE residual magnitude. Used by `AdaptiveSampler`.
...
├── CollocationSampler.sample()    ← produce collocation points (config-driven)
├── DataCallback.transform_data()  ← optional transforms (e.g., DataScaling)
└── setup()
        │
        ▼
InferredContext
├── domain: Domain (N-dimensional bounds + step sizes)
...
2. Create the data module: `class MyDataModule(PINNDataModule)` implementing `gen_data()` in `anypinn/catalog/`. Collocation generation is handled by the sampler selected via `TrainingDataConfig.collocation_sampler`.
...
- Protocols for callable interfaces (`ODECallable`, `LogFn`, `PredictDataFn`, `CollocationSampler`, `ResidualScorer`)
