...
def reference_solution(
    ...
    """Compute clean reference u(x,y,t) and v(x,y,t) via method-of-lines."""
    import numpy as np
    ...
    def rhs(_t: float, y: np.ndarray) -> np.ndarray:
        ...
        uv2 = u * v**2
        du_dt = true_du * lap_u - uv2 + true_f * (1 - u)
        dv_dt = true_dv * lap_v + uv2 - (true_f + true_k) * v
    ...
    sol = solve_ivp(
        ...
        t_eval=t_eval,
        method="RK45",
        max_step=dx**2 / (4 * max(true_du, true_dv)),
    )

    u_sol = sol.y[: n * n, :].reshape(n, n, -1)
    v_sol = sol.y[n * n :, :].reshape(n, n, -1)

    t_norm_sim = sol.t / T_TOTAL
...
class GrayScott2DDataModule(PINNDataModule):
    ...
    def gen_data(self, config: GenerationConfig) -> tuple[Tensor, Tensor]:
        ...
        u_ref = torch.tensor(u_np, dtype=torch.float32)
        v_ref = torch.tensor(v_np, dtype=torch.float32)
